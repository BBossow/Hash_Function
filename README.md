...
Hash Function 3:
    ...
    Originally I didn't use a static num and instead the length of the stringData, which was effecient for title but forQuote table it wasn't since some of these quotes where very long causing the loc to go back to the begining causing more collisions and a longer runtime. So I switched it to a static number that wasn't too big or small which allowed for efficentency to increase for both 
...
The method that worked the best was my third hash function. I feel like this method worked so good because each character has its own distinct unicode and the amount of chars in each StringData isn't always the same. So the odds of two titles or quotes having the same key after the function is pretty low which allows for it to work very efficently. 
